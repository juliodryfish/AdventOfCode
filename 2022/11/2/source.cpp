...
int main() {
    ...
    // Solved using common denominator to lower the item numbers so they fit
    // https://www.splashlearn.com/math-vocabulary/addition/common-denominator#:~:text=We%20can%20obtain%20common%20denominators,to%20get%204%E2%81%8410%20
    int math_trick = 1;
    std::for_each(std::begin(monkeys), std::end(monkeys), [&math_trick](auto & monkey){
        math_trick *= monkey.inspect;
    });
    std::cout << "math_trick: " << math_trick << std::endl;
    for(int i = 0; i < 10000; ++i) {
        std::for_each(std::begin(monkeys), std::end(monkeys), [&math_trick, &monkeys](auto & monkey){
            std::for_each(std::begin(monkey.items), std::end(monkey.items), [&math_trick, &monkey, &monkeys](auto item){
                monkey.inspects++;
                if(!monkey.operation[0].compare("+")) {
                    if(!monkey.operation[1].compare("old")) {
                        item = item + item;
                    } else {
                        item = item + std::stoll(monkey.operation[1]);
                    }
                } else { // "*"
                    if(!monkey.operation[1].compare("old")) {
                        item = item * item;
    ...
    long long result = monkeys[0].inspects * monkeys[1].inspects;
