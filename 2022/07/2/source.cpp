...
int main() {
    ...
    int result = directories["/"];
    int space = (30 * 1000 * 1000) - ((70 * 1000 * 1000) - directories["/"]);
    std::for_each(std::begin(directories), std::end(directories), [&result, &space](auto & dir){
        auto & size = dir.second;
        if(size >= space && size < result) result = size;
    });
